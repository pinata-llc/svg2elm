...
const app = Elm.Main.init({ flags: { argv: process.argv, versionMessage: "0.1.0" } });
...
app.ports.readFiles.subscribe((paths: string[]) => {
  app.ports.filesRead.send(paths.map(p => [path.basename(p).split(".")[0], fs.readFileSync(p).toString()]));
});
